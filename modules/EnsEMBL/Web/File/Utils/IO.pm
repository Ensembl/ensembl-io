...
### @return Hashref (nice mode) or Boolean
  ...
  if ($args->{'nice'}) {
    ...
    if (-e $path && -f $path) {
      ...
      throw("File $path could not be found: $!") unless $args->{'no_exception'};
      return 0;
    }
  }
}
...
### @return Hashref (nice mode) or Boolean
  ...
  if ($args->{'nice'}) {
    ...
    if ($success) {
      ...
      throw("File $path could not be created: $exception") unless $args->{'no_exception'};
      return 0;
    }
  }
} 
...
### @return Hashref (in nice mode) or Boolean
  ...
  if ($args->{'nice'}) {
    ...
    if (unlink $path) {
      ...
      throw("Error occurred when deleting file $path: $!") unless $args->{'no_exception'};
      return 0;
    }
  }
}
...
### @return Hashref (in nice mode) or Boolean 
  ...
  if (!$content) {
    if ($args->{'nice'}) {
      ...
      throw(sprintf qq(No content given for file '%s'.), $path) unless $args->{'no_exception'};
      return 0;
    }
  }
  ...
  if ($has_path) { 
    ...
    if ($args->{'nice'}) {
      ...
      if ($@) {
        throw(sprintf qq(Could not create path '%s'.), $path) unless $args->{'no_exception'};
        return 0;
      }
      else {
        return 1;
...
### @return Hashref (in nice mode) or Boolean
  ...
  if (ref($lines) ne 'ARRAY') {
    if ($args->{'nice'}) {
      ...
      throw(sprintf qq(Input for '%s' must be an arrayref.), $path) unless $args->{'no_exception'};
      return 0;
    }
  }
  ...
  if ($args->{'nice'}) {
    ...
    if ($@) {
      throw(sprintf qq(Could not append lines to file '%s'.), $path) unless $args->{'no_exception'};
      return 0;
    }
    else {
      return 1;
